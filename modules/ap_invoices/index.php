...
<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
  ...
  <div class="flex flex-wrap gap-3">
    ...
    <button type="button" id="submitSelectedInvoicesBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed" disabled>
      Create PR from selected invoices
    </button>
  ...
  <div class="bg-white rounded-xl border overflow-hidden">
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <?php if (empty($invoices)): ?>
        <tr>
          <td colspan="<?= hasRole('admin', 'maker', 'finance_manager') ? 18 : 17 ?>" class="px-4 py-8 text-center text-gray-400">No invoices matched your search.</td>
        </tr>
        <?php endif; ?>
      </tbody>
      <?php if (!empty($invoices)): ?>
      <tfoot>
        <tr class="bg-gray-50 text-xs font-semibold text-gray-600 border-t">
          <td colspan="<?= hasRole('admin', 'maker', 'finance_manager') ? 9 : 8 ?>" class="px-4 py-2">Total (<?= count($invoices) ?> records)</td>
